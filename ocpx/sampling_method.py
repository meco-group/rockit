...
class SamplingMethod:
    ...
    def discrete_system(self, stage):
        ...
        X = [X0]
        intg = getattr(self, "intg_"+self.intg)(f, X0, U)
        for j in range(self.M):
            intg_res = intg(x0=X[-1], u=U,DT=DT)
    ...
    def intg_rk(self, f, X, U):
        ...
        return Function('F', [X, U, DT], [X + DT/6*(k1 + 2*k2 + 2*k3 + k4), poly_coeff], ['x0', 'u', 'DT'], ['xf', 'poly_coeff'])

    def intg_cvodes(self, f, X, U):
        # A single CVODES step
        data, opts = self.prepare_sundials(f, X, U)
        I = integrator('intg_cvodes', 'cvodes', data, opts)
        DT = MX.sym("DT")
        return Function('F', [X, U, DT], [I.call({'x0': X, 'p': vertcat(U, DT)})['xf'], MX()], ['x0', 'u', 'DT'], ['xf', 'poly_coeff'])

    def intg_idas(self, f, X, U):
        # A single IDAS step
        data, opts = self.prepare_sundials(f, X, U)
